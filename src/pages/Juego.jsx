...
export default function Juego() {
  ...
  useEffect(() => {
    ...
    const unsub = onValue(salaRef, (snap) => {
      ...
      if (data.estado === "revision") {
        navigate(`/sala/${salaId}/revision`);
      }
      if (data.estado === "lobby") {
        navigate(`/sala/${salaId}`);
      }
      if (data.estado === "finalizado") {
        navigate(`/sala/${salaId}/resultados`);
      }
    });
    return () => unsub();
  }, [salaId, navigate]);
  ...
  const elegirLetra = async (letra) => {
    await update(ref(db, `salas/${salaId}`), { letra });
  };
            ...
            <div className="grid grid-cols-6 gap-2 max-w-md mb-6">
              {LETRAS.map((l) => (
                <button
                  ...
                  className="w-12 h-12 rounded-lg bg-slate-800 hover:bg-emerald-500 font-bold"
                >
                  {l}
                </button>
              ))}
